...
def convert_single_example(ex_index, example, max_seq_length, tokenizer):
    ...
    truncate_seq_pair(ids_a, ids_b, max_seq_length-3)  # account for [CLS], [SEP], [SEP]
    ...
    #     tf.logging.info("token_type_ids: %s" % " ".join([str(x) for x in token_type_ids]))
    #     tf.logging.info("label: %s (id = %d)" % (example.label, label_id))
    feature = InputFeatures(input_ids=input_ids, input_mask=input_mask, token_type_ids=token_type_ids, label_id=label_id, is_real_example=True)
    return feature
...
class DataProcessor(object):
    ...
    def get_train_examples(self, train_query_ids_file, trainset_dir, dev_ids, mode):
        queryids_dict = self.read_ids_file(train_query_ids_file)
        files = os.listdir(trainset_dir)  # trainset
        examples = []
        for file in files:
            if "." in file:
                continue
            ...
            with open(infile) as f:
                for ind, line in enumerate(f):
                    ...
                    if ind % 50000 == 0:
                        print(qid, golden_id)
                    # 训练集
                    if mode == "train" and qid in dev_ids:
                        continue
                    # 验证集
                    if mode == "dev" and qid not in dev_ids:
                        continue

                    # 查看id是否在train_query_ids中
                    ...
                    if golden_id not in self.corpus_ids:
                        continue
                    examples.append(InputExample(qid+"##"+golden_id, text_a=query_ids, text_b=self.corpus_ids[golden_id], label=1))
                    if mode == "train":
                        num_chosed = min(len(doc_ids.split("#")), 4)
                    else:
                        num_chosed = len(doc_ids.split("#"))
                    chosed_docids = random.sample(doc_ids.split("#"), num_chosed)  # 第三列是存储负样例的
                    for docid in chosed_docids:
                        if docid not in self.corpus_ids:
                            continue
                        if docid == golden_id or docid == query_ids:
                            continue
                        docids = self.corpus_ids[docid]
                        examples.append(InputExample(qid+"##"+docid, text_a=query_ids, text_b=docids, label=0))
        return examples


    def get_dev_examples(self, train_query_ids_file, trainset_dir, dev_ids, mode):
        queryids_dict = self.read_ids_file(train_query_ids_file)
        ...
        for file in files:
            ...
            with open(infile) as f:
                for ind, line in enumerate(f):
                    ...
                    if golden_id not in self.corpus_ids:
                        continue
                    temp_examples.append(InputExample(qid+"##"+golden_id, text_a=query_ids, text_b=self.corpus_ids[golden_id], label=1))
                    if mode == "train":
                        num_chosed = min(len(doc_ids.split("#")), 4)
                    else:
                        num_chosed = len(doc_ids.split("#"))
                    ...
                    for docid in chosed_docids:
                        ...
                        if docid == golden_id or docid == query_ids:
                            continue
                        docids = self.corpus_ids[docid]
                        temp_examples.append(InputExample(qid+"##"+docid, text_a=query_ids, text_b=docids, label=0))
    ...
    def get_features(self, examples, max_seq_length):
        ...
        for (index, example) in enumerate(examples):
            feature = convert_single_example(index, example, max_seq_length, self.tokenizer)
